...
export const GlareCard = ({
  ...
  return (
    <div
      ...
      ref={refElement}
      onPointerMove={(event) => {
        // Don't rotate when interacting with buttons
        if (
    ...
    >
      {/* Added pointer-events-auto to make sure clicks pass through */}
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden pointer-events-auto">
        {/* This div contains the actual content and should receive clicks */}
        <div
          className={cn(
            "h-full w-full bg-slate-950 pointer-events-auto z-10 relative",
            className
          )}
        >
          {children}
        ...
        {/* These are decorative layers and should not intercept clicks */}
